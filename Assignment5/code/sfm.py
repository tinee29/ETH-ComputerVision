...
def main():
  ...
  e_im1.SetPose(*identity_pose)

  max_points = 0
  best_pose = None

  for R, t in possible_relative_poses:
      # Set the second image's pose for each possible pose
      e_im2.SetPose(R, t)
      ...
      count_in_front = sum([np.dot(R @ p + t, p) > 0 and np.dot(p, p) > 0 for p in points3D])

      # Update the best pose if this one has more points in front
      if count_in_front > max_points:
          max_points = count_in_front
          best_pose = (R, t)

  # Set the best pose found for the second image
  if best_pose is not None:
      e_im2.SetPose(*best_pose)
  else:
      raise Exception("Could not find a valid pose")
