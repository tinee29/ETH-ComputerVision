...
def UpdateReconstructionState(new_points3D, corrs, points3D, images):
  ...
  # Append new 3D points to the existing points
  updated_points3D = np.vstack([points3D, new_points3D])

  for im_name, local_corrs in corrs.items():
      # Calculate global correspondences indices
      global_corrs = [len(points3D) + i for i in local_corrs]

      # Add the correspondences to the images
      images[im_name].Add3DCorrs(global_corrs, list(range(new_points3D.shape[0])))

  return updated_points3D, images