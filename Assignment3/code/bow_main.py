...
def grid_points(img, nPointsX, nPointsY, border):
    ...
    """
    h, w = img.shape  # Get the height and width of the input image

    # Calculate the step size for the regular grid points
    step_x = int((w - 2 * border) / (nPointsX - 1))
    step_y = int((h - 2 * border) / (nPointsY - 1))

    # Initialize an empty list to store the grid points
    vPoints = []

    # Generate the grid points
    for y in range(border, h - border, step_y):
        for x in range(border, w - border, step_x):
            vPoints.append([x, y])
    ...
    for i in range(len(vPoints)):
        ...
        for cell_y in range(-2, 2):
            for cell_x in range(-2, 2):
                ...
                # Get the gradient magnitudes and orientations for the current cell
                ...
                cell_grad_x = cv2.normalize(cell_grad_x, None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
                cell_grad_y = cv2.normalize(cell_grad_y, None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)

                # Compute gradient magnitude and orientation
                magnitude, angle = cv2.cartToPolar(cell_grad_x, cell_grad_y, angleInDegrees=True)

                # Create histogram of gradient orientations for the current cell
                hist, _ = np.histogram(angle, bins=nBins, range=(0, 180), weights=magnitude)
...
def bow_histogram(vFeatures, vCenters):
    ...
    assert vFeatures.shape[1] == vCenters.shape[1], "Feature and center dimensions must match"
    
    # Get the number of clusters (visual words)
    num_clusters = vCenters.shape[0]
    
    # Initialize the histogram with zeros
    histo = np.zeros(num_clusters)

    # Assign each descriptor to the closest cluster center
    labels = findnn(vFeatures, vCenters)[0]
...
def create_bow_histograms(nameDir, vCenters):
    ...
    for i in tqdm(range(nImgs)):
        ...
        # Calculate the bag-of-words histogram for the current image
...
def bow_recognition_nearest(histogram,vBoWPos,vBoWNeg):
    ...
    # Calculate distances between the test histogram and histograms of positive and negative training images
    DistPos = np.linalg.norm(histogram - vBoWPos, axis=1)
    DistNeg = np.linalg.norm(histogram - vBoWNeg, axis=1)

    # Find the nearest neighbor in the positive and negative sets and decide based on this neighbor
    nearest_pos = np.argmin(DistPos)
    nearest_neg = np.argmin(DistNeg)


    if DistPos[nearest_pos] < DistNeg[nearest_neg]:
        sLabel = 1  
    else:
        sLabel = 0  
